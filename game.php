...
//todo: Move a lot of this logic to PingPongGame class
...
if(isset($game) && isset($_POST['new_round'])){
  ...
  if($status === 1){
    //status will be 1 when someone won
    ...
    $playerWon = true;
    $victoriousPlayer = $game->players[0];  //the last player must be the winner right?

    header('Location: scoreboard.php?game_id=' . $game->gameId);  //rederict player to scoreboard of this game
    exit;
  }
